...
API_KEY = os.getenv("AIzaSyA0E0ifJ3WtiVDsnyj-VPang2pHn6JXoKQ") # Add your API key here as a string
...
def get_video_title(youtube, video_id):
    try:
        ...
        logging.error(f"Error getting video title for {video_id}: {e}")
        return None
    ...
    for playlist_id in PLAYLIST_IDS.keys():
        try:
            ...
            for item in response['items']:
                ...
                if video_title:
                    latest_videos.append((playlist_id, video_title, video_url))
                else:
                    # Fallback to the title from the API if get_video_title fails
...
def update_obsidian_daily_note_and_videos_md(videos):
    ...
    for video in videos:
        playlist_id, video_title, video_url = video
        if playlist_id in categorized_videos:
            categorized_videos[playlist_id].append((video_title, video_url))
        else:
    ...
    for playlist_id, videos in categorized_videos.items():
        ...
        if os.path.exists(file_path):
            with open(file_path, 'r') as file:
                ...
                cached_links = {line.strip() for line in content if line.startswith('[')}

        new_videos = [(title, url) for title, url in videos if url not in cached_links]
